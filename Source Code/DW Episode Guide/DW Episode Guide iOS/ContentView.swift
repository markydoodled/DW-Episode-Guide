...
struct ContentView: View {
        ...
        List {
            ForEach(searchResults, id: \.self) { result in
                ...
                if result == "An Unearthly Child" {
                    ...
                    i = 334
                } else if result == "The Power Of The Doctor" {
                    i = 335
                }
                }) {
                    Text("\(result)")
    ...
    var setting: some View {
        ...
        if horizontalSizeClass == .regular {
                Form {
                    Section(header: Label("Misc.", systemImage: "ellipsis.circle"), footer: Text("© Mark Howard 2023, All Rights And Images Owned By The BBC")) {
                    HStack {
                        Text("Version")
                        Spacer()
                        Text("1.3.2")
