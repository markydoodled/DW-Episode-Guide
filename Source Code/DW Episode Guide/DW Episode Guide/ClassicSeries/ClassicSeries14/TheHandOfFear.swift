...
struct TheHandOfFear: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheHandOfFearClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheHandOfFearClass>
        @State var showingShare = false
        @AppStorage("TheHandOfFearNotes") var notes = ""
        @AppStorage("TheHandOfFearWatched") var watched: Bool = false
            ...
            ScrollView {
                HStack {
                    ...
                    Image("TheHandOfFear")
                        ...
                        .onDrag {
                            let data = NSImage(named: "TheHandOfFear")?.tiffRepresentation
                        ...
                        Image("TheHandOfFear")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheHandOfFear")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheHandOfFear")! as UIImage)
                            } preview: {Image("TheHandOfFear")}
                        Spacer()
                    }
                    Text("Story No. 87")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 87") as NSString)
                    }
                    Text("Written By - Bob Baker\nAnd Dave Martin")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Hand_of_Fear")!], applicationActivities: nil)
                        }
                    }
...
struct TheHandOfFear_Previews: PreviewProvider {
