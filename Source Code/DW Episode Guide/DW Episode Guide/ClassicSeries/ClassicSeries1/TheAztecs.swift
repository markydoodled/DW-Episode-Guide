...
struct TheAztecs: View {
    @Environment(\.managedObjectContext) private var viewContext
    
    @FetchRequest(entity: TheAztecsClass.entity(),
        sortDescriptors: [],
        animation: .default)
    private var items: FetchedResults<TheAztecsClass>
    @State var showingShare = false
    @AppStorage("TheAztecsNotes") var notes = ""
    @AppStorage("TheAztecsWatched") var watched: Bool = false
    #if os(iOS)
                    ...
                    Image("TheAztecs")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheAztecs")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheAztecs")?.tiffRepresentation
                        ...
                        Image("TheAztecs")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheAztecs")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheAztecs")! as UIImage)
                            } preview: {Image("TheAztecs")}
                        Spacer()
                    }
                    Text("Story No. 6")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 6") as NSString)
                    }
                    Text("Written By - John Lucarotti")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - John Lucarotti") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Aztecs_(Doctor_Who)")!], applicationActivities: nil)
                        }
                    }
...
struct TheAztecs_Previews: PreviewProvider {
