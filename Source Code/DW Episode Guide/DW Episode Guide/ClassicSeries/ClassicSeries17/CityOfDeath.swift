...
struct CityOfDeath: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: CityOfDeathClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<CityOfDeathClass>
            ...
            ScrollView {
                HStack {
                    ...
                    Image("CityOfDeath")
                        ...
                        .onDrag {
                            let data = NSImage(named: "CityOfDeath")?.tiffRepresentation
                        ...
                        Image("CityOfDeath")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "CityOfDeath")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "CityOfDeath")! as UIImage)
                            } preview: {Image("CityOfDeath")}
                        Spacer()
                    }
                    Text("Story No. 105")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 105") as NSString)
                    }
                    Text("Written By - 'David Agnew'\n(Pseudonym For\nDavid Fisher,\nDouglas Adams\nAnd Graham Williams)")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - 'David Agnew'\n(Pseudonym For\nDavid Fisher,\nDouglas Adams\nAnd Graham Williams)") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/City_of_Death")!], applicationActivities: nil)
                        }
                    }
...
struct CityOfDeath_Previews: PreviewProvider {
