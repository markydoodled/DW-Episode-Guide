...
struct InvasionOfTheDinosaurs: View {
    @Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: InvasionOfTheDinosaursClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<InvasionOfTheDinosaursClass>
        @State var showingShare = false
        @AppStorage("InvasionOfTheDinosaursNotes") var notes = ""
    @AppStorage("InvasionOfTheDinosaursWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("InvasionOfTheDinosaurs")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "InvasionOfTheDinosaurs")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "InvasionOfTheDinosaurs")?.tiffRepresentation
            ...
            .toolbar {
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/Invasion_of_the_Dinosaurs")!]))
                }
            }
                        ...
                        Image("InvasionOfTheDinosaurs")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "InvasionOfTheDinosaurs")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "InvasionOfTheDinosaurs")! as UIImage)
                            } preview: {Image("InvasionOfTheDinosaurs")}
                        Spacer()
                    }
                    Text("Story No. 71")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 71") as NSString)
                    }
                    Text("Written By - Malcolm Hulke")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Malcolm Hulke") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Invasion_of_the_Dinosaurs")!], applicationActivities: nil)
                        }
                    }
...
struct InvasionOfTheDinosaurs_Previews: PreviewProvider {
