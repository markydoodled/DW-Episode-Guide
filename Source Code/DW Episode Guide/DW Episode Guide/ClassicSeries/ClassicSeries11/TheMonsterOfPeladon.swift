...
struct TheMonsterOfPeladon: View {
    @Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheMonsterOfPeladonClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheMonsterOfPeladonClass>
        @State var showingShare = false
        @AppStorage("TheMonsterOfPeladonNotes") var notes = ""
    @AppStorage("TheMonstersOfPeladonWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("TheMonsterOfPeladon")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheMonsterOfPeladon")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheMonsterOfPeladon")?.tiffRepresentation
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Monster_of_Peladon")!]))
                }
            }
                        ...
                        Image("TheMonsterOfPeladon")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheMonsterOfPeladon")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheMonsterOfPeladon")! as UIImage)
                            } preview: {Image("TheMonsterOfPeladon")}
                        Spacer()
                    }
                    Text("Story No. 73")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 73") as NSString)
                    }
                    Text("Written By - Brian Hayles")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Brian Hayles") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Monster_of_Peladon")!], applicationActivities: nil)
                        }
                    }
...
struct TheMonsterOfPeladon_Previews: PreviewProvider {
