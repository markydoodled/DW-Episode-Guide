...
struct TheSpacePirates: View {
    @Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheSpacePiratesClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheSpacePiratesClass>
        @State var showingShare = false
        @AppStorage("TheSpacePiratesNotes") var notes = ""
    @AppStorage("TheSpacePiratesWatched") var watched: Bool = false
    #if os(iOS)
            ...
            ScrollView {
                HStack {
                    ...
                    Image("TheSpacePirates")
                        ...
                        .onDrag {
                            let data = NSImage(named: "TheSpacePirates")?.tiffRepresentation
                        ...
                        Image("TheSpacePirates")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheSpacePirates")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheSpacePirates")! as UIImage)
                            } preview: {Image("TheSpacePirates")}
                        Spacer()
                    }
                    Text("Story No. 49")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 49") as NSString)
                    }
                    Text("Written By - Robert Holmes")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Space_Pirates")!], applicationActivities: nil)
                        }
                    }
...
struct TheSpacePirates_Previews: PreviewProvider {
