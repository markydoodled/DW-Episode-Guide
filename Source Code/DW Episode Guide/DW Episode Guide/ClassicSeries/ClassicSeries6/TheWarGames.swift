...
struct TheWarGames: View {
    @Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheWarGamesClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheWarGamesClass>
        @State var showingShare = false
        @AppStorage("TheWarGamesNotes") var notes = ""
    @AppStorage("TheWarGamesWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
            ScrollView {
                HStack {
                    ...
                    Image("TheWarGames")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheWarGames")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheWarGames")?.tiffRepresentation
                    ...
                    Text("Story No. 50")
                            .font(.title3)
                        Text("Written By - Terrance Dicks\nAnd Malcolm Hulke")
                            .font(.title3)
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_War_Games")!]))
                }
            }
                        ...
                        Image("TheWarGames")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheWarGames")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheWarGames")! as UIImage)
                            } preview: {Image("TheWarGames")}
                        Spacer()
                    }
                    Text("Story No. 50")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 50") as NSString)
                    }
                    Text("Written By - Terrance Dicks\nAnd Malcolm Hulke")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Terrance Dicks\nAnd Malcolm Hulke") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_War_Games")!], applicationActivities: nil)
                        }
                    }
...
struct TheWarGames_Previews: PreviewProvider {
