...
struct TheTimeMonster: View {
    @Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheTimeMonsterClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheTimeMonsterClass>
        @State var showingShare = false
        @AppStorage("TheTimeMonsterNotes") var notes = ""
    @AppStorage("TheTimeMonsterWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("TheTimeMonster")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheTimeMonster")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheTimeMonster")?.tiffRepresentation
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Time_Monster")!]))
                }
            }
                        ...
                        Image("TheTimeMonster")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheTimeMonster")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheTimeMonster")! as UIImage)
                            } preview: {Image("TheTimeMonster")}
                        Spacer()
                    }
                    Text("Story No. 64")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 64") as NSString)
                    }
                    Text("Written By - Robert Sloman")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Robert Sloman") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Time_Monster")!], applicationActivities: nil)
                        }
                    }
...
struct TheTimeMonster_Previews: PreviewProvider {
