...
struct TheSeedsOfDoom: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheSeedsOfDoomClass.entity(),
            sortDescriptors: [],
            animation: .default)
        ...
        ForEach(items) { item in
                    ...
                    Image("TheSeedsOfDoom")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheSeedsOfDoom")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheSeedsOfDoom")?.tiffRepresentation
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Seeds_of_Doom")!]))
                }
            }
                        ...
                        Image("TheSeedsOfDoom")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheSeedsOfDoom")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheSeedsOfDoom")! as UIImage)
                            } preview: {Image("TheSeedsOfDoom")}
                        Spacer()
                    }
                    Text("Story No. 85")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 85") as NSString)
                    }
                    Text("Written By - Robert Banks Stewart")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Robert Banks Stewart") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Seeds_of_Doom")!], applicationActivities: nil)
                        }
                    }
...
struct TheSeedsOfDoom_Previews: PreviewProvider {
