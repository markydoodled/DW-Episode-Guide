...
struct ParadiseTowers: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: ParadiseTowersClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<ParadiseTowersClass>
        @State var showingShare = false
        @AppStorage("ParadiseTowersNotes") var notes = ""
        @AppStorage("ParadiseTowersWatched") var watched: Bool = false
    #if os(iOS)
                            ...
                            Image("ParadiseTowers")
                                ...
                                .contextMenu {
                                    Button(action: {let pasteboard = NSPasteboard.general
                                        ...
                                        pasteboard.writeObjects([NSImage(named: "ParadiseTowers")!])
                                    }) {
                                        Text("Copy")
                                    }
                                }
                                .onDrag {
                                    let data = NSImage(named: "ParadiseTowers")?.tiffRepresentation
                        ...
                        Image("ParadiseTowers")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "ParadiseTowers")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "ParadiseTowers")! as UIImage)
                            } preview: {Image("ParadiseTowers")}
                        Spacer()
                    }
                    Text("Story No. 145")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 145") as NSString)
                    }
                    Text("Written By - Stephen Wyatt")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Paradise_Towers")!], applicationActivities: nil)
                        }
                    }
...
struct ParadiseTowers_Previews: PreviewProvider {
