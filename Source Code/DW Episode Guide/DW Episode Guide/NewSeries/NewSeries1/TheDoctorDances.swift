...
struct TheDoctorDances: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheDoctorDancesClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheDoctorDancesClass>
        @State var showingShare = false
        @AppStorage("TheDoctorDancesNotes") var notes = ""
        @AppStorage("TheDoctorDancesWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                            ...
                            Image("TheDoctorDances")
                                ...
                                .contextMenu {
                                    Button(action: {let pasteboard = NSPasteboard.general
                                        ...
                                        pasteboard.writeObjects([NSImage(named: "TheDoctorDances")!])
                                    }) {
                                        Text("Copy")
                                    }
                                }
                                .onDrag {
                                    let data = NSImage(named: "TheDoctorDances")?.tiffRepresentation
                    ...
                    ToolbarItem(placement: .primaryAction) {
                        ...
                        ToolbarItem(placement: .primaryAction) {
                            ...
                            .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Doctor_Dances")!]))
                        }
                    }
                        ...
                        Image("TheDoctorDances")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheDoctorDances")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheDoctorDances")! as UIImage)
                            } preview: {Image("TheDoctorDances")}
                        Spacer()
                    }
                    Text("Story No. 164b")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 164b") as NSString)
                    }
                    Text("Written By - Steven Moffat")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Doctor_Dances")!], applicationActivities: nil)
                        }
                    }
...
struct TheDoctorDances_Previews: PreviewProvider {
