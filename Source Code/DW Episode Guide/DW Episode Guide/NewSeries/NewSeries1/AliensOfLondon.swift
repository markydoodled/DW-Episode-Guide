...
struct AliensOfLondon: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: AliensOfLondonClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<AliensOfLondonClass>
        @State var showingShare = false
        @AppStorage("AliensOfLondonNotes") var notes = ""
        @AppStorage("AliensOfLondonWatched") var watched: Bool = false
    #if os(iOS)
                            ...
                            Image("AliensOfLondon")
                                ...
                                .contextMenu {
                                    Button(action: {let pasteboard = NSPasteboard.general
                                        ...
                                        pasteboard.writeObjects([NSImage(named: "AliensOfLondon")!])
                                    }) {
                                        Text("Copy")
                                    }
                                }
                                .onDrag {
                                    let data = NSImage(named: "AliensOfLondon")?.tiffRepresentation
                        ...
                        Image("AliensOfLondon")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "AliensOfLondon")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "AliensOfLondon")! as UIImage)
                            } preview: {Image("AliensOfLondon")}
                        Spacer()
                    }
                    Text("Story No. 160a")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 160a") as NSString)
                    }
                    Text("Written By - Russell T Davies")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Aliens_of_London")!], applicationActivities: nil)
                        }
                    }
...
struct AliensOfLondon_Previews: PreviewProvider {
