//
//  ThePowerOfTheDoctor.swift
//  DW Episode Guide
//
//  Created by Mark Howard on 17/05/2023.
//

import SwiftUI
import UniformTypeIdentifiers

struct ThePowerOfTheDoctor: View {
    @Environment(\.managedObjectContext) private var viewContext

            @FetchRequest(entity: ThePowerOfTheDoctorClass.entity(),
                sortDescriptors: [],
                animation: .default)
            private var items: FetchedResults<ThePowerOfTheDoctorClass>
            @State var showingShare = false
            @AppStorage("ThePowerOfTheDoctorNotes") var notes = ""
            @AppStorage("ThePowerOfTheDoctorWatched") var watched: Bool = false
...
ForEach(items) { item in
            ...
            Image("ThePowerOfTheDoctor")
                ...
                .contextMenu {
                    Button(action: {let pasteboard = NSPasteboard.general
                        ...
                        pasteboard.writeObjects([NSImage(named: "ThePowerOfTheDoctor")!])
                    }) {
                        Text("Copy")
                    }
                }
                .onDrag {
                    let data = NSImage(named: "ThePowerOfTheDoctor")?.tiffRepresentation
    ...
    ToolbarItem(placement: .primaryAction) {
        ...
        ToolbarItem(placement: .primaryAction) {
            ...
            .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Power_of_the_Doctor")!]))
        }
    }
                ...
                Image("ThePowerOfTheDoctor")
                    ...
                    .contextMenu {
                        Button(action: {let pasteboard = UIPasteboard.general
                            pasteboard.image = UIImage(named: "ThePowerOfTheDoctor")
                        }) {
                            Label("Copy", systemImage: "doc.on.doc")
                        }
                    }
                    .onDrag {
                        return NSItemProvider(object: UIImage(named: "ThePowerOfTheDoctor")! as UIImage)
                    } preview: {Image("ThePowerOfTheDoctor")}
                Spacer()
            }
            Text("Story No. 300")
            .onDrag {
                return NSItemProvider(object: String("Story No. 300") as NSString)
            }
            Text("Written By - Chris Chibnall")
                .onDrag {
                    return NSItemProvider(object: String("Written By - Chris Chibnall") as NSString)
                }
        ...
        ToolbarItem(placement: .navigationBarTrailing) {
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                ...
                .sheet(isPresented: $showingShare) {
                    ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Power_of_the_Doctor")!], applicationActivities: nil)
                }
            }
...
struct ThePowerOfTheDoctor_Previews: PreviewProvider {
