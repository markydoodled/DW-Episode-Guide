...
struct TheAlmostPeople: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: TheAlmostPeopleClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<TheAlmostPeopleClass>
        @State var showingShare = false
        @AppStorage("TheAlmostPeopleNotes") var notes = ""
        @AppStorage("TheAlmostPeopleWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
            ScrollView {
                HStack {
                    ...
                    Image("TheAlmostPeople")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "TheAlmostPeople")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "TheAlmostPeople")?.tiffRepresentation
                    ...
                    Text("Story No. 217b")
                            .font(.title3)
                        Text("Written By - Matthew Graham")
                            .font(.title3)
                }
                    Spacer()
                }
            ...
            .toolbar {
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/The_Almost_People")!]))
                }
            }
                        ...
                        Image("TheAlmostPeople")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "TheAlmostPeople")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "TheAlmostPeople")! as UIImage)
                            } preview: {Image("TheAlmostPeople")}
                        Spacer()
                    }
                    Text("Story No. 217b")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 217b") as NSString)
                    }
                    Text("Written By - Matthew Graham")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Matthew Graham") as NSString)
                        }
                ...
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/The_Almost_People")!], applicationActivities: nil)
                        }
                    }
...
struct TheAlmostPeople_Previews: PreviewProvider {
