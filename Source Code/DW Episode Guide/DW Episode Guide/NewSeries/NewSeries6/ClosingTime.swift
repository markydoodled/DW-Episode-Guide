...
struct ClosingTime: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: ClosingTimeClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<ClosingTimeClass>
        @State var showingShare = false
        @AppStorage("ClosingTimeNotes") var notes = ""
        @AppStorage("ClosingTimeWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("ClosingTime")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "ClosingTime")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "ClosingTime")?.tiffRepresentation
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/Closing_Time_(Doctor_Who)")!]))
                }
            }
                        ...
                        Image("ClosingTime")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "ClosingTime")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "ClosingTime")! as UIImage)
                            } preview: {Image("ClosingTime")}
                        Spacer()
                    }
                    Text("Story No. 223")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 223") as NSString)
                    }
                    Text("Written By - Gareth Roberts")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Gareth Roberts") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Closing_Time_(Doctor_Who)")!], applicationActivities: nil)
                        }
                    }
...
struct ClosingTime_Previews: PreviewProvider {
