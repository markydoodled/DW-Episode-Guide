...
struct LetsKillHitler: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: LetsKillHitlerClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<LetsKillHitlerClass>
        @State var showingShare = false
        @AppStorage("LetsKillHitlerNotes") var notes = ""
        @AppStorage("LetsKillHitlerWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("LetsKillHitler")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "LetsKillHitler")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "LetsKillHitler")?.tiffRepresentation
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/Let%27s_Kill_Hitler")!]))
                }
            }
                        ...
                        Image("LetsKillHitler")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "LetsKillHitler")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "LetsKillHitler")! as UIImage)
                            } preview: {Image("LetsKillHitler")}
                        Spacer()
                    }
                    Text("Story No. 219")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 219") as NSString)
                    }
                    Text("Written By - Steven Moffat")
                        .onDrag {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Let%27s_Kill_Hitler")!], applicationActivities: nil)
                        }
                    }
...
struct LetsKillHitler_Previews: PreviewProvider {
