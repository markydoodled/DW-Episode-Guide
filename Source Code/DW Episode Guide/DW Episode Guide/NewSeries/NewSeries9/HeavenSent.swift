...
struct HeavenSent: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: HeavenSentClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<HeavenSentClass>
        @State var showingShare = false
        @AppStorage("HeavenSentNotes") var notes = ""
        @AppStorage("HeavenSentWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
                    ...
                    Image("HeavenSent")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "HeavenSent")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "HeavenSent")?.tiffRepresentation
            ...
            .toolbar {
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/Heaven_Sent_(Doctor_Who)")!]))
                }
            }
                        ...
                        Image("HeavenSent")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "HeavenSent")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "HeavenSent")! as UIImage)
                            } preview: {Image("HeavenSent")}
                        Spacer()
                    }
                    Text("Story No. 261")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 261") as NSString)
                    }
                    Text("Written By - Steven Moffat")
                        .onDrag {
                ...
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Heaven_Sent_(Doctor_Who)")!], applicationActivities: nil)
                        }
                    }
...
struct HeavenSent_Previews: PreviewProvider {
