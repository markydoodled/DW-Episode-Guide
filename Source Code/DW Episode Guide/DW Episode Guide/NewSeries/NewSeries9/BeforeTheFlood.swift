...
struct BeforeTheFlood: View {
@Environment(\.managedObjectContext) private var viewContext

        @FetchRequest(entity: BeforeTheFloodClass.entity(),
            sortDescriptors: [],
            animation: .default)
        private var items: FetchedResults<BeforeTheFloodClass>
        @State var showingShare = false
        @AppStorage("BeforeTheFloodNotes") var notes = ""
        @AppStorage("BeforeTheFloodWatched") var watched: Bool = false
    #if os(iOS)
        ...
        ForEach(items) { item in
            ScrollView {
                HStack {
                    ...
                    Image("BeforeTheFlood")
                        ...
                        .contextMenu {
                            Button(action: {let pasteboard = NSPasteboard.general
                                ...
                                pasteboard.writeObjects([NSImage(named: "BeforeTheFlood")!])
                            }) {
                                Text("Copy")
                            }
                        }
                        .onDrag {
                            let data = NSImage(named: "BeforeTheFlood")?.tiffRepresentation
                    ...
                    Text("Story No. 255b")
                            .font(.title3)
                        Text("Written By - Toby Whithouse")
                            .font(.title3)
                }
                    Spacer()
                }
            ...
            ToolbarItem(placement: .primaryAction) {
                ...
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .background(SharingsPicker(isPresented: $showingShare, sharingItems: [URL(string: "https://en.wikipedia.org/wiki/Before_the_Flood_(Doctor_Who)")!]))
                }
            }
                        ...
                        Image("BeforeTheFlood")
                            ...
                            .contextMenu {
                                Button(action: {let pasteboard = UIPasteboard.general
                                    pasteboard.image = UIImage(named: "BeforeTheFlood")
                                }) {
                                    Label("Copy", systemImage: "doc.on.doc")
                                }
                            }
                            .onDrag {
                                return NSItemProvider(object: UIImage(named: "BeforeTheFlood")! as UIImage)
                            } preview: {Image("BeforeTheFlood")}
                        Spacer()
                    }
                    Text("Story No. 255b")
                    .onDrag {
                        return NSItemProvider(object: String("Story No. 255b") as NSString)
                    }
                    Text("Written By - Toby Whithouse")
                        .onDrag {
                            return NSItemProvider(object: String("Written By - Toby Whithouse") as NSString)
                        }
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        ...
                        .sheet(isPresented: $showingShare) {
                            ActivityView(activityItems: [URL(string: "https://en.wikipedia.org/wiki/Before_the_Flood_(Doctor_Who)")!], applicationActivities: nil)
                        }
                    }
...
struct BeforeTheFlood_Previews: PreviewProvider {
